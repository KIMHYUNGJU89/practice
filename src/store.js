...
let cartList = createSlice({
    ...
    reducers: {
        inCartList(state, action) {
            let locate = state.findIndex((item) => item.id == action.payload.id);
            let isDuplicate = state.some(cartItem => cartItem.id === action.payload.id);
    
            if (locate !== -1 && state[locate].count !== null) {
                state[locate].count += 1;
            } else if (!isDuplicate) {
                state.push(action.payload);
            } else {
                console.error("Invalid payload or duplicate item:", action.payload);
            }
...
export let { inCartList, plusCount, minusCount, changeList, deleteItem } = cartList.actions;
